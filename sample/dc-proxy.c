    ...
    struct sockaddr *a, int slen, void *p)
{
	...
	SSL_CTX *b_in_ctx = (SSL_CTX *)p;
	SSL * ssl_in = SSL_new(b_in_ctx);
	b_in = bufferevent_openssl_socket_new(base, fd, ssl_in, BUFFEREVENT_SSL_ACCEPTING, BEV_OPT_CLOSE_ON_FREE|BEV_OPT_DEFER_CALLBACKS);
	...
	SSL_CTX *b_out_ctx = SSL_CTX_new(SSLv23_client_method());
        SSL *ssl = SSL_new(b_out_ctx);
